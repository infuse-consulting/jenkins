...
node {
    stage('Read and Execute tests') {
        ...
        withCredentials([
            ...
            echo "Running tests in project ${params['ProjectID']} with tags ${params['Tags']}"
                            ...
                            dir("${env.WORKSPACE}\\${tests[index].Id}") {
                                ...
                                if (httpCode.contains("200")) {
                                    ...
                                    try {
                                        ...
                                        if (run_id != null) {
                                            testResults[count] = "TestName: '${tests[index].Name}' ${datasetType} (Passed) - ${APP_WEBSITE_URL}/p/${params['ProjectID']}/executions/${run_id}"
                                        } else {
                                            testResults[count] = "TestName: '${tests[index].Name}' ${datasetType} (Failed) - ${isMultiDataset ? 'multidataset_run.log' : 'run.log' } not generated"
                                        }
                                    } catch(Exception ex) {
                                        String run_id = getRunId()
                                        testResults[count] = "TestName: '${tests[index].Name}' ${datasetType} (Failed) - Exception occured: ${ex.getMessage()} - ${APP_WEBSITE_URL}/p/${params['ProjectID']}/executions/${run_id}"
                                    } finally{
...
def getTests(String baseUrl) {
    ...
    echo "Retrieved the following tests from project ${params['ProjectID']} with the tags ${params['Tags']} and status ${params['Status']}"
    while(true) {
        URL url = new URL("${baseUrl}/projects/${params['ProjectID']}/testindex?tags=${params['Tags']}&status=${params['Status']}&cursor=${cursor}")
...
def getScenarios(String baseUrl, String testId){
    ...
    if (runWithDataset) {
        ...
        if (isScenarioPresentForTest) {
            URL url = new URL("${baseUrl}/projects/${params['ProjectID']}/tests/${testId}/scenarios")
...
boolean scenariosPresent(String baseUrl, String testId) {
    URL url = new URL("${baseUrl}/projects/${params['ProjectID']}/tests/${testId}")
...
def getDefaultEnv(String baseUrl) {
    URL url = new URL("${baseUrl}/projects/${params['ProjectID']}/environments/default");
...
def getEnvByName(String baseUrl, String envName) {
    ...
    while (fetchEnvironmentPage) {
        URL url = addQueryParameterToUrl("${baseUrl}/projects/${params['ProjectID']}/environments",  [
                q: envName,
                cursor: cursor
        ])
